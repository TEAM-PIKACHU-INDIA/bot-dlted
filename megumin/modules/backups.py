...
import megumin.modules.sql.notes_sql as sql
from megumin import dispatcher, LOGGER, OWNER_ID, SUDO_USERS, MESSAGE_DUMP
from megumin.__main__ import DATA_IMPORT
from megumin.modules.helper_funcs.chat_status import user_admin
from megumin.modules.helper_funcs.misc import build_keyboard, revert_buttons
from megumin.modules.helper_funcs.msg_types import get_note_type
from megumin.modules.rules import get_rules
import megumin.modules.sql.rules_sql as rulessql
from megumin.modules.sql import warns_sql as warnssql
import megumin.modules.sql.blacklist_sql as blacklistsql
from megumin.modules.sql import disable_sql as disabledsql
from megumin.modules.sql import cust_filters_sql as filtersql
import megumin.modules.sql.welcome_sql as welcsql
import megumin.modules.sql.locks_sql as locksql
from megumin.modules.connection import connected
...
def export_data(bot: Bot, update: Update, chat_data):
    ...
    f=open("megumin{}.backup".format(chat_id), "w")
    ...
    try:
        ...
        pass
    bot.sendDocument(current_chat_id, document=open('megumin{}.backup'.format(chat_id), 'rb'), caption="*Successfully imported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This  is specially made for notes.".format(chat.title, chat_id, tgl), timeout=360, reply_to_message_id=msg.message_id, parse_mode=ParseMode.MARKDOWN)
    os.remove("megumin{}.backup".format(chat_id)) # Cleaning file
